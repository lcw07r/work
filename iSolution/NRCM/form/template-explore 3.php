...
<div class="pageContent">
	...
	//register form -> search form -> search result
	if (isset($_POST['submit'])){     
		initialise();    //If the register form is submited: validate input, check required fields, set error message when applied;
		
		if (array_filter($err)) {
			outputRegisterForm();  //If has error found, out put the register form again (with error messages) ;
		}else {
			if (isset($_POST['submit'])){     
				initialise();    //If the search form is submited: validate input, check required fields, set error message when applied;
		
				if (array_filter($err)) {
					outputRegisterForm();  //If has error found, out put the search form again (with error messages) ;
				}else {
					outputSearchForm();  // if pass all, out put the search result.
				}
			}else{
				outputSearchForm();  // if pass register form check and the search form is not submitted, out put the search form.
			}
		}
	}else {
		outputRegisterForm();  //If the register form is not submited yet, out put the register form;
	}
